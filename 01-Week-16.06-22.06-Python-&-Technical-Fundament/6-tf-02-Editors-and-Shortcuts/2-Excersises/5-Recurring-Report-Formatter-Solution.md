...
Process used to convert Format X into Format Y:

- `Ctrl + N` to open a new file.
- `Ctrl + V` to paste the raw Format X data.
- `Ctrl + H` to open Find & Replace.
- `Alt + R` to enable regex search.
- Find pattern: `^([^;]+);([^;]+);(.+)$`
- Replace with: `* **\1:** \2 (Status: \3)`
- `Ctrl + Alt + Enter` to replace all matches.

For repeatability, the process was saved as the macro `convert_format_x_to_y.sublime-macro` and bound to this shortcut:

```json
{ "keys": ["ctrl+shift+y"], "command": "run_macro_file", "args": { "file": "Packages/User/convert_format_x_to_y.sublime-macro" } }
```

The platform screenshot shows the assignment submitted and graded as Done.

![Platform evidence](screenshots/tf2-05-platform-submission.png)
