...
**Goal:**  
Explain the standard WEP-cracking workflow, name a suitable tool, and summarize why WEP is fundamentally broken even when the local capture file is missing.

**Requirements:**

- Use a legitimate analysis tool appropriate for WEP capture testing.

- Explain the cracking process at a high level.

- Answer why WEP is insecure regardless of password complexity.

- Avoid fabricating the specific key if the source pcap is not available locally.
...
objective = 'Explain the standard WEP-cracking workflow, name a suitable tool, and summarize why WEP is fundamentally broken even when the local capture file is missing.'
evidence = 'Submit the final result in the exact format requested by the task.'

# Main logic
Source-truth note:

The referenced `wep.pcap` file is not present in the local repository, so the exact recovered WEP key cannot be produced honestly from the current folder alone.

Suitable tool and example command:

aircrack-ng wep.pcap

High-level process:

- Collect many WEP-encrypted packets.
- Extract enough IV-related material from the traffic.
- Use statistical attacks against RC4/WEP key scheduling weaknesses.
- Recover the WEP key once enough relevant packets are available.

Direct conceptual answers:

- WEP is fundamentally insecure because its RC4 implementation and IV design are broken. The small IV space repeats quickly, and the key scheduling process leaks enough structure for statistical attacks.
- Password complexity does not fix the protocol weakness, because the attack targets the cryptographic design and repeated IV behavior, not only weak human-chosen passwords.
- WEP was superseded by WPA and then WPA2 as stronger standards.
```

**Alternative (compact):**

```text
WEP fails because the protocol design is broken, not merely because users choose weak passwords.
```
