...
Part 1 parity checks:

- `11010110` -> number of `1` bits: `5` -> odd -> false
- `00111001` -> number of `1` bits: `4` -> even -> correct
- `10101011` -> number of `1` bits: `5` -> odd -> false
- `01100011` -> number of `1` bits: `4` -> even -> correct

Part 2:

```python
def has_even_parity(packet_string):
    return packet_string.count('1') % 2 == 0
```
...
```python
def generate_even_parity_bit(data_bits_string):
    if data_bits_string.count('1') % 2 == 0:
        return '0'
    else:
        return '1'
```
